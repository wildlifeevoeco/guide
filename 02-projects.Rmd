...
* `man/` holds the documentation of your functions, usually automatically generated with [{roxygen2}](https://roxygen2.r-lib.org/)
...
of the {targets} manual has a great overview of two approaches to managing interconnected analysis steps and data: 
...
Read the link above, then see the [section on {targets}](https://weel.gitlab.io/guide/targets.html).
...
[project oriented workflow](https://rstats.wtf/projects). The `R/` folder is used for functions. Functions help us chunk out 
...
[Efficient R / Binary file formats](https://csgillespie.github.io/efficientR/input-output.html#binary-file-formats). Or if you use `data.table`, use `fread` and `fwrite` to read/write CSV files. If your dates are stored in the ISO standard format, they will be automatically converted to `IDate` by `fread`. 
...
Note, you can change an open source license for an existing project but typically you need to ask permission from everyone else that has contributed. (Not legal advice) but more [here](https://opensource.stackexchange.com/questions/33/how-can-a-project-be-relicensed) and [here](https://copyright.universityofcalifornia.edu/ownership/joint-works.html) to start. 
...
* [{targets} manual](https://books.ropensci.org/targets/functions.html)
